...
func (ru *RoleUpdate) Save(ctx context.Context) (int, error) {
	if v, ok := ru.mutation.Rolename(); ok {
		if err := role.RolenameValidator(v); err != nil {
			return 0, &ValidationError{Name: "rolename", err: fmt.Errorf("ent: validator failed for field \"rolename\": %w", err)}
		}
	}
...
func (ru *RoleUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if nodes := ru.mutation.RemovedUserIDs(); len(nodes) > 0 {
...
func (ruo *RoleUpdateOne) Save(ctx context.Context) (*Role, error) {
	if v, ok := ruo.mutation.Rolename(); ok {
		if err := role.RolenameValidator(v); err != nil {
			return nil, &ValidationError{Name: "rolename", err: fmt.Errorf("ent: validator failed for field \"rolename\": %w", err)}
		}
	}
...
func (ruo *RoleUpdateOne) sqlSave(ctx context.Context) (r *Role, err error) {
	...
	if nodes := ruo.mutation.RemovedUserIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   role.UserTable,
