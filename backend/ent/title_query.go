...
func (tq *TitleQuery) QueryUser() *UserQuery {
...
func (tq *TitleQuery) WithUser(opts ...func(*UserQuery)) *TitleQuery {
	query := &UserQuery{config: tq.config}
...
func (tq *TitleQuery) sqlAll(ctx context.Context) ([]*Title, error) {
	...
	if query := tq.withUser; query != nil {
		...
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(title.UserColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.title_user
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "title_user" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "title_user" returned %v for node %v`, *fk, n.ID)
			}
