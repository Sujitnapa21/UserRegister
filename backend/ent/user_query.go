...
func (uq *UserQuery) QuerySex() *SexQuery {
	query := &SexQuery{config: uq.config}
...
//  WithSex tells the query-builder to eager-loads the nodes that are connected to
// the "sex" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithSex(opts ...func(*SexQuery)) *UserQuery {
	query := &SexQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if uq.withSex != nil || uq.withRole != nil || uq.withTitle != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := uq.withSex; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].sex_user; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "sex_user" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Sex = n
			}
	...
	if query := uq.withRole; query != nil {
		...
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].role_user; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
	...
	if query := uq.withTitle; query != nil {
		...
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].title_user; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "title_user" returned %v`, n.ID)
			}
			for i := range nodes {
