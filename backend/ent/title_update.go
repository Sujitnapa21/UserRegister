...
func (tu *TitleUpdate) AddUser(u ...*User) *TitleUpdate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
func (tu *TitleUpdate) RemoveUser(u ...*User) *TitleUpdate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
func (tu *TitleUpdate) Save(ctx context.Context) (int, error) {
	if v, ok := tu.mutation.Titlename(); ok {
		if err := title.TitlenameValidator(v); err != nil {
			return 0, &ValidationError{Name: "titlename", err: fmt.Errorf("ent: validator failed for field \"titlename\": %w", err)}
		}
	}
...
func (tu *TitleUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if nodes := tu.mutation.RemovedUserIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   title.UserTable,
			Columns: []string{title.UserColumn},
			Bidi:    false,
...
// AddUser adds the user edges to User.
func (tuo *TitleUpdateOne) AddUser(u ...*User) *TitleUpdateOne {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
// RemoveUser removes user edges to User.
func (tuo *TitleUpdateOne) RemoveUser(u ...*User) *TitleUpdateOne {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
func (tuo *TitleUpdateOne) Save(ctx context.Context) (*Title, error) {
	if v, ok := tuo.mutation.Titlename(); ok {
		if err := title.TitlenameValidator(v); err != nil {
			return nil, &ValidationError{Name: "titlename", err: fmt.Errorf("ent: validator failed for field \"titlename\": %w", err)}
		}
	}
...
func (tuo *TitleUpdateOne) sqlSave(ctx context.Context) (t *Title, err error) {
	...
	if nodes := tuo.mutation.RemovedUserIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   title.UserTable,
